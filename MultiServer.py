...
class Context:
    def __init__(self, host, port, password, adminpassword = None):
...
async def process_client_cmd(ctx : Context, client : Client, cmd, args):
    ...
    if cmd == 'Say':
        ...
        args = args.split()
        if args[0] == '!admin':
            if ctx.adminpassword == args[1]:
                client.admin = True
                notify_all(ctx, '[Server]: ' + client.name + ' has become an admin')
            else:
                notify_client(client, '[Server]: Wrong admin password')
            return 

        notify_all(ctx, client.name + ': ' + " ".join(args))
...
async def main():
    ...
    parser.add_argument('--adminpassword', default=None)
    parser.add_argument('--disableconsole', default=False, action='store_true')
    args = parser.parse_args()

    ctx = Context(args.host, args.port, args.password, args.adminpassword)
